...
One isolated three-slice run completed on 2026-08-12 outside the
loopback-restricted sandbox, and the current provider-URL root slice passed
locally on 2026-08-23. Repeated final-source runs and hosted CI evidence are
still pending; local fixture passes are not a release claim.
...
| Launch Contract validation                        | Verified (local) | Valid contracts use zero model calls; malformed contract-shaped input fails closed                                                                               | Founder review of the first real contract                                 |
| Rough-prose and product-build Codex CLI hosts     | Verified (local) | Internally owned hosts use bounded stdin context and a safe environment projection; provider credentials/transports stay separate                                | Complete the first real build; retain OS-isolation limits                 |
| Typed Launch Contract                             | Verified (local) | Reviewable proposition hypothesis, strict 15-field capability map and invariants, supported deterministic projections, canonical rendering and digest            | Founder review plus commercial and live proof for the first real contract |
...
| Definitive root-CLI Golden Path                   | Fixture verified | Initial GitHub/Neon/Stripe/Vercel graph succeeds on a fixture provider URL; exact apply replay adds no product/provider invocation; optional roles stay deferred | Repeated final-source/CI runs, then live accounts and URL                 |
...
| Production deployment and custom domain           | Fixture verified | Initial provider-URL launch succeeds without a domain; requested custom DNS remains a deferred nonblocking action                                                | Reachable Vercel URL plus separate domain read-back if requested          |
| Primary journey, launch report and Launch Receipt | Fixture verified | Product-specific fixture journey plus sanitized local report/receipt, including deferred follow-up; generic smoke stays unverified                               | Live journey against the deployed URL and provider evidence               |
...
| Surface                                              | Status           | Product Truth                   | Boundary                                                                             |
| ---------------------------------------------------- | ---------------- | ------------------------------- | ------------------------------------------------------------------------------------ |
| Secret/PII/claim/migration/graph invariants          | Verified (local) | truth-003, truth-030, truth-032 | Final branch still needs the complete frozen release gate and CI evidence            |
| Desktop/mobile product composition and accessibility | Fixture verified | truth-014, truth-029            | Not a production accessibility certification or guarantee for arbitrary generated UI |
| Raw HTML, metadata, sitemap, robots and consent      | Fixture verified | truth-014, truth-041            | Must rerun against each child and its deployed production URL                        |
| Public repository/community/security files           | Verified (local) | truth-030                       | GitHub security/ruleset settings require external read-back                          |
| Public stable tag/package                            | Planned          | truth-036                       | This remains alpha until the first real founder launch is dogfooded                  |
